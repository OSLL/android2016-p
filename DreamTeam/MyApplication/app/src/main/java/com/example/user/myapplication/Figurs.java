...
public class Figurs {
    ...
    public HashMap<String, Integer> int_colors = new HashMap<>();
    // float s = 2.5f;
    ...
    public void DrawKrug(Canvas canvas, float s, float center_x, float center_y, boolean haveBlackR) {
        ...
        Draw1F4(100*s, canvas, center_x, center_y);
        canvas.rotate(90, center_x, center_y);
        just4color.setColor(Color.parseColor("#E13D63"));
        Draw1F4(100*s, canvas, center_x, center_y);
        Paint paint = new Paint();
    ...
    public void DrawBlock(Canvas canvas, int center_x, int center_y, int color) {
        int a = 0, b = 0;
        boolean fl1 = false;
        if (!fl1) {
            a = canvas.getWidth()/3;
