...
public class NumActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!getIntent().getBooleanExtra("two", false)) {
            ...
            // Запись в таблицу результатов играков
            try {
                SQLiteOpenHelper helper = new DBHelper(this);
                SQLiteDatabase db = helper.getReadableDatabase();
                ContentValues value = new ContentValues();
                // Данные о 1-м игроке
                value.put("NAME", AppActivity.name1String);
                value.put("SCORE", AppActivity.score1);
                // Записали данные в БД
                db.insert("PLAYERTABLE", null, value);
                // Данные о 2-м игроке
                value.put("NAME", AppActivity.name2String);
                value.put("SCORE", AppActivity.score2);
                // Записали данные в БД
                db.insert("PLAYERTABLE", null, value);
                db.close();
            } catch (SQLiteException e) {
